...
static Options options = {
    .viewSizeX = 130, .viewSizeY = 240, .soundSeed = 9, .isDarkColor = false
};

#define GAME_CENTER_X (options.viewSizeX / 4.0f)
#define GAME_CENTER_Y (options.viewSizeY / 4.0f)
...
static void update() {
  ...
  for (int i = 0; i < HEXMIN_LANE_COUNT; i++) {
    ...
    vectorSet(&p, GAME_CENTER_X, GAME_CENTER_Y);
  ...
  for (int j = 0; j < HEXMIN_LANE_COUNT; j++) {
    ...
    if (lanes[j] == 4) {
      ...
      if (oa == 0 || oa == 2) {
        play(COIN);
        addScore((int)multiplier, GAME_CENTER_X, GAME_CENTER_Y);
        multiplier += 6;
        lanes[j] = 0;
      }
    }
