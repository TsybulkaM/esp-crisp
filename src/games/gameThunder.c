...
static Options options = {
    .viewSizeX = 130, .viewSizeY = 230, .soundSeed = 9, .isDarkColor = true};
...
static void update() {
  ...
  FOR_EACH(lines, i) {
    ...
    if (l->isActive) {
      color = YELLOW;
      line(l->from.x - cameraX, l->from.y, l->to.x - cameraX, l->to.y);
    ...
    if (l->ticks > 0) {
      ...
      if (activeTicks < 0 && (l->to.y > 90 || lc >= THUNDER_MAX_LINE_COUNT)) {
        ...
        color = YELLOW;
        for (int j = 0; j < 99; j++) {
          particle(al->to.x - cameraX, al->to.y, 30, 2, 0, M_PI * 2);
          al->isActive = true;
          al = al->prevLine;
          if (al == NULL) {
            break;
      ...
      play(HIT);
      color = BLACK;
      particle(l->to.x - cameraX, l->to.y, 9, 1, 0, M_PI * 2);
  ...
  if (character(pc, player.x - cameraX, 87).isColliding.rect[YELLOW]) {
    play(RANDOM);
    gameOver();
  }
  ...
  FOR_EACH(stars, i) {
    ...
    if (s->pos.y > 89 && s->vy > 0) {
     ...
     bool *c = character("a", s->pos.x - cameraX, s->pos.y).isColliding.character;
    if (c['b'] || c['c']) {
      play(COIN);
