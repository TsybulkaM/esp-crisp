...
#define CHARACTER_SPACE (int)((CHARACTER_WIDTH * SCALE) / 2)
...
// Screen positioning macros
//! Get center X coordinate of the view
#define CENTER_X() (options.viewSizeX / 2.0f)
//! Get center Y coordinate of the view
#define CENTER_Y() (options.viewSizeY / 2.0f)
//! Get X coordinate as percentage of view width (0.0 - 1.0)
#define VIEW_X(percent) (options.viewSizeX * (percent))
//! Get Y coordinate as percentage of view height (0.0 - 1.0)
#define VIEW_Y(percent) (options.viewSizeY * (percent))
//! Get X coordinate relative to center (offset from center)
#define FROM_CENTER_X(offset) (CENTER_X() + (offset))
//! Get Y coordinate relative to center (offset from center)
#define FROM_CENTER_Y(offset) (CENTER_Y() + (offset))

// Mode management
typedef void (*ModeUpdateFunc)(void);
EXTERNC void registerMode(ModeUpdateFunc updateFunc); // Register mode with its update function
EXTERNC bool checkDoubleClick(ButtonState* button);
EXTERNC void nextMode();
EXTERNC int getCurrentMode();
EXTERNC int getModeCount();
EXTERNC void setMode(int mode);
EXTERNC void updateCurrentMode(); // Execute current mode's update function
