...
static ModeUpdateFunc modeUpdateFuncs[MAX_MODES];

void registerMode(ModeUpdateFunc updateFunc) {
  if (modeCount < MAX_MODES) {
    modeUpdateFuncs[modeCount] = updateFunc;
    modeCount++;
  }
}

bool checkDoubleClick(ButtonState* button) {
  static int lastPressTicks = -100;
  if (button->isJustPressed) {
    if (ticks - lastPressTicks < 20) { // Within ~0.3 seconds
      lastPressTicks = -100;
      return true;
    }
...
void updateCurrentMode() {
  if (currentMode >= 0 && currentMode < modeCount && modeUpdateFuncs[currentMode] != NULL) {
    modeUpdateFuncs[currentMode]();
  }
}
...
Collision rect(float x, float y, float w, float h) {
  ...
  beginAddingRects();
  addRect(false, x * SCALE, y * SCALE, w * SCALE, h * SCALE, &hitCollision);
  endAddingRects();
  return hitCollision;
}
...
Collision box(float x, float y, float w, float h) {
  ...
  initCollision(&hitCollision);
  beginAddingRects();
  addRect(true, x * SCALE, y * SCALE, w * SCALE, h * SCALE, &hitCollision);
  endAddingRects();
  return hitCollision;
}
...
Collision line(float x1, float y1, float x2, float y2) {
  ...
  initCollision(&hitCollision);
  beginAddingRects();
  drawLine(x1 * SCALE, y1 * SCALE, (x2 - x1) * SCALE, (y2 - y1) * SCALE, &hitCollision);
  endAddingRects();
  return hitCollision;
}
...
Collision bar(float x, float y, float length, float angle) {
  ...
  Vector p;
  vectorSet(&p, x * SCALE - l.x * barCenterPosRatio, y * SCALE - l.y * barCenterPosRatio);
  beginAddingRects();
...
Collision arc(float centerX, float centerY, float radius, float angleFrom,
  ...
  int lc = ceilf(ao * sqrtf(scaledRadius * 0.25f));
  if (lc < 1) {
    lc = 1;
  } else if (lc > 36) {
    lc = 36;
  ...
  float ai = ao / lc;
  float a = af;
  float p1x = scaledRadius * cosf(a) + scaledCenterX;
  float p1y = scaledRadius * sinf(a) + scaledCenterY;
  float p2x, p2y;
  float ox, oy;
  for (int i = 0; i < lc; i++) {
    a += ai;
    p2x = scaledRadius * cosf(a) + scaledCenterX;
    p2y = scaledRadius * sinf(a) + scaledCenterY;
    ox = p2x - p1x;
    oy = p2y - p1y;
...
static Collision drawCharacters(char *msg, float x, float y, bool _hasCollision,
  ...
  initCollision(&hitCollision);
  int ml = strlen(msg);
  x = x * SCALE - CHARACTER_WIDTH / 2;
  y = y * SCALE - CHARACTER_HEIGHT / 2;
  for (int i = 0; i < ml; i++) {
...
static void parseDescription() {
  ...
  descriptionX = (viewSizeX - (dl * CHARACTER_WIDTH) - ((dl - 1) * CHARACTER_SPACE)) / 2;
}
...
static void updateTitle() {
  ...
  drawCharacters(
    title, 
    (viewSizeX-(titleLen*CHARACTER_WIDTH)-(titleLen-1)*CHARACTER_SPACE)/2,
    viewSizeY * 0.25f,
    false,
    true
  );
  if (ticks > 30) {
    for (int i = 0; i < descriptionLineCount; i++) {
      drawCharacters(
        descriptions[i], 
        descriptionX,
        options.viewSizeY * 0.30f + i * CHARACTER_HEIGHT,
        false,
        true
      );
    }
...
static void drawGameOver() {
  uint8_t gameOverLen = strlen(gameOverText);
  drawCharacters(
    gameOverText,
    (viewSizeX-(gameOverLen*CHARACTER_WIDTH)-(gameOverLen-1)*CHARACTER_SPACE)/2,
    viewSizeY * 0.40f,
    false,
    true
  );
